...
class Statute:
    ...
    # ) -> str:
    #     """
    #     Get formatted text for a statute, optionally restricted to a subsection label path.

    #     Args:
    #         subsection: Dot-separated label path, e.g., "A.2.b". If None, returns the full statute.
    #         indent: Number of spaces per indentation level.
    #         headers: Controls label display:
    #             "none" -> omit labels entirely
    #             "normal" -> show only local label ("2. Foo")
    #             "verbose" -> show full hierarchical label ("A.2.b. Foo")
    #     """
    ...
    #             header_str = f"{label}. " if label else ""
    #         elif headers == "verbose":
    #             full_label = ".".join(parent_labels + [label]) if label else ".".join(parent_labels)
    #             header_str = f"{full_label}. " if full_label else ""
    ...
    #         for child in section.get("subsections", []):
    #             child_labels = parent_labels + [label] if label else parent_labels
    #             lines.append(format_section(child, level + 1, child_labels))
